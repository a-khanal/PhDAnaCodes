...
public class ekpkpreqkmavgvz_data_v1 {
	//public int NFTElec;
	...
	*/
	...
	public DataGroup dg_kinematics, dg_efKp, dg_sKpKm, dg_rec_electron, dg_rec_kp1, dg_rec_kp2, dg_rec_km, dg_rec_xi, dg_rec_p, dg_rec_pim, dg_vtime, dg_fdtof, dg_cdtof, dg_cdPart, dg_vz, dg_tof_t, dg_tof_path, dg_counter;
	...
	public ekpkpreqkmavgvz_data_v1() {
	...
	//	VB = new LorentzVector(0, 0, Eb, Eb);
		...
		fn_lam_fit.setParameters(29.97000, 1.11800, 0.03481) // for f18outfte data set
		//fn_lam_fit = setParameter(0, BigDecimal 29.97000);
		//fn_lam_fit = setParameter(1, BigDecimal 1.11800);
		//fn_lam_fit = setParameter(2, BigDecimal 0.03481);

		//fn_sig_fit = new F1D("fn-sig-fit", "[amp]*gaus(x,[mean],[sigma])", 0, 5);
		fn_sig_fit = new F1D("fn-sig-fit", "[a]*gaus(x,[m],[s])", 0, 5);
		fn_sig_fit.setParameters(13.81000, 1.19700, 0.03000) // for f18outfte data set
		...
		fn_lamsig_fit.setParameters(32.1178, 1.11847, 0.0393565, 13.7672, 1.20246, 0.030000);// for f18outfte data set
		//fn_lamsig_fit.setParameters(87.0000, 1.11255, 0.030158, 37.0000, 1.18968, 0.030000);// for f18infte data set
		//fn_lamsig_fit.setParameters(87.0000, 1.10467, 0.0330015, 54.0000, 1.17859, 0.030000);// for s19infte data set
		fn_lam_fit = new F1D("fn-lam-fit", "[a]*gaus(x,[m],[s])", 0, 5);
		fn_lam_fit.setParameters(32.1178, 1.11847, 0.0393565); // for f18outfte data set
		//fn_lam_fit.setParameters(87.0000, 1.11255, 0.030158); // for f18infte data set
		//fn_lam_fit.setParameters(87.0000, 1.10467, 0.0330015); // for s19infte data set
		fn_sig_fit = new F1D("fn-sig-fit", "[a]*gaus(x,[m],[s])", 0, 5);
		fn_sig_fit.setParameters(13.7672, 1.20246, 0.030000); // for f18outfte data set
		//fn_sig_fit.setParameters(37.0000, 1.18968, 0.030000); // for f18infte data set
		//fn_sig_fit.setParameters(54.0000, 1.17859, 0.030000); // for s19infte data set
		fn_bg_fit = new F1D("fn-bg-fit", "[a]+[b]*x", 0, 5);
		fn_bg_fit.setParameters(-24.2009, 32.5685); // for f18outfte data set
		//fn_bg_fit.setParameters(-54.5086, 71.8595); // for f18infte data set
		//fn_bg_fit.setParameters(-58.1102, 77.1365); // for s19infte data set
	...
	// FT electron overview
		...
		hi_virphoton.setTitle("");

		dg_kinematics = new DataGroup(2,2);
		dg_kinematics.addDataSet(hi_FT_Q2, 0);
		dg_kinematics.addDataSet(hi_FT_W, 1);
		dg_kinematics.addDataSet(hi_FT_W_Q2, 2);
		dg_kinematics.addDataSet(hi_virphoton, 3);
	...
	// KP1
		...
		dg_efKp = new DataGroup(3, 2);
		dg_efKp.addDataSet(hi_FT_e_p_the, 0);
		dg_efKp.addDataSet(hi_FT_e_p_f, 1);
		dg_efKp.addDataSet(hi_FT_e_t_f, 2);
		dg_efKp.addDataSet(hi_rec_kp1_p_the, 3);
		dg_efKp.addDataSet(hi_rec_kp1_p_phi, 4);
		dg_efKp.addDataSet(hi_rec_kp1_the_phi, 5);
	...
	//KM
		...
		dg_sKpKm = new DataGroup(3, 2);
		dg_sKpKm.addDataSet(hi_rec_kp2_p_the, 0);
		dg_sKpKm.addDataSet(hi_rec_kp2_p_phi, 1);
		dg_sKpKm.addDataSet(hi_rec_kp2_the_phi, 2);
		dg_sKpKm.addDataSet(hi_rec_km_p_the, 3);
		dg_sKpKm.addDataSet(hi_rec_km_p_phi, 4);
		dg_sKpKm.addDataSet(hi_rec_km_the_phi, 5);
...
//FD smearing functions (From Giovanni/Florian) to match MC and data resolution derived using elastic event
	...
	public int makeFTElectron(DataBank bank, DataBank recFT, DataBank recftResponse) {
		...
		for (int k = 0; k < bank.rows(); k++) {
			...
			if (ftbpid == 11 && q == -1 && inFT  && e_mom > 1.3 && e_mom < 4.5 && e_the < 4.5 && e_the > 2.5) { //&& e_mom > 1.3 && e_mom < 3.9 && e_mom < 4.5
				NFTElec++;
...
//			hi_FT_e_p_the.fill(e_the, e_mom);
		...
		/*
			...
			*/
			...
			//System.out.println(mm_fkpskpkm + " " + w_lam + " " + w_sig + " " + w_bg + " " + w_lamsig);
			//*/
			//0.03 <= q2 && q2 <= 0.13

			if (1.00 <= mm_fkpskpkm && mm_fkpskpkm <= 1.29 && e_theta >=2.5 && e_theta <=4.5 && e_p >=1.0 && e_p <= 4.3 && fkp_theta >= 5.0 && fkp_theta <= 35.0 && skp_theta >= 5.0 && skp_theta<= 35.0 && km_theta>= 5.0 && km_theta <= 35.0){

				hi_FT_W_Q2.fill(W, q2);
				hi_FT_W.fill(W);
				hi_FT_Q2.fill(q2);	
				hi_virphoton.fill(nu);

				hi_FT_e_t_f.fill(e_phi, e_theta);
				hi_FT_e_p_f.fill(e_phi, e_p);
				hi_FT_e_p_the.fill(e_theta, e_p);

				hi_rec_kp1_p_the.fill(fkp_theta, fkp_p);
				hi_rec_kp1_p_phi.fill(fkp_phi, fkp_p);
				hi_rec_kp1_the_phi.fill(fkp_phi, fkp_theta);

				hi_rec_kp2_p_the.fill(skp_theta, skp_p);
				hi_rec_kp2_p_phi.fill(skp_phi, skp_p);
				hi_rec_kp2_the_phi.fill(skp_phi, skp_theta);

				hi_rec_km_p_the.fill(km_theta, km_p);
				hi_rec_km_p_phi.fill(km_phi, km_p);
				hi_rec_km_the_phi.fill(km_phi, km_theta);
			}
			if (e_theta >=2.5 && e_theta <=4.5 && e_p >=1.0 && e_p <= 4.3 && fkp_theta >= 5.0 && fkp_theta <= 35.0 && skp_theta >= 5.0 && skp_theta<= 35.0 && km_theta>= 5.0 && km_theta <= 35.0){
				///*
				//Store three kaon event information in the arraylist so that we can implement event mixing technique later on
            	//public static ArrayList<LorentzVector> lv_my_ftels, lv_my_fast_fkps, lv_my_slow_fkps, lv_my_fkms;
            	lv_my_ftels.add(p_fte.vector());
...
// */
	...
	public void plot() {
		...
		myCanvas = new EmbeddedCanvasTabbed("Kinematics", "efKp", "sKpKm","REC-electron","electron-res","REC-kp1(fast)","kp1(fast-kp)-res","REC-kp2(slow)","kp2(slow-kp)-res","REC-km","km-res","REC-Xi","MM-ekpkpkm","MM-ekpkpkm1","MM-scatter","MM-lam-evnt","IM-kmlambda","fkpckp","MM-spectra", "FD-TOF", "CD-TOF", "CD-Part", "VTime", "particle-Vz","TOF-t","TOF-path", "Counter", "Request");
		...
		myCanvas.getCanvas("efKp").divide(3, 2);
		myCanvas.getCanvas("efKp").setSize(1600, 1000);
		myCanvas.getCanvas("efKp").setGridX(false);
		myCanvas.getCanvas("efKp").setGridY(false);
		myCanvas.getCanvas("efKp").setAxisFontSize(18);
		myCanvas.getCanvas("efKp").setAxisTitleSize(24);
		myCanvas.getCanvas("efKp").draw(dg_efKp);
		myCanvas.getCanvas("efKp").getPad(0).getAxisZ().setLog(true);
		myCanvas.getCanvas("efKp").getPad(1).getAxisZ().setLog(true);
		myCanvas.getCanvas("efKp").getPad(2).getAxisZ().setLog(true);
		myCanvas.getCanvas("efKp").getPad(3).getAxisZ().setLog(true);
		myCanvas.getCanvas("efKp").getPad(4).getAxisZ().setLog(true);
		myCanvas.getCanvas("efKp").getPad(5).getAxisZ().setLog(true);


		myCanvas.getCanvas("sKpKm").divide(3, 2);
		myCanvas.getCanvas("sKpKm").setSize(1600, 1000);
		myCanvas.getCanvas("sKpKm").setGridX(false);
		myCanvas.getCanvas("sKpKm").setGridY(false);
		myCanvas.getCanvas("sKpKm").setAxisFontSize(18);
		myCanvas.getCanvas("sKpKm").setAxisTitleSize(24);
		myCanvas.getCanvas("sKpKm").draw(dg_sKpKm);
		myCanvas.getCanvas("sKpKm").getPad(0).getAxisZ().setLog(true);
		myCanvas.getCanvas("sKpKm").getPad(1).getAxisZ().setLog(true);
		myCanvas.getCanvas("sKpKm").getPad(2).getAxisZ().setLog(true);
		myCanvas.getCanvas("sKpKm").getPad(3).getAxisZ().setLog(true);
		myCanvas.getCanvas("sKpKm").getPad(4).getAxisZ().setLog(true);
		myCanvas.getCanvas("sKpKm").getPad(5).getAxisZ().setLog(true);
	...
	*/
	...
	public static void main(String[] args) {
		...
		int count = 0;
		//long maxevents = 8000;
		//int maxevents = 1000000/2.5; // for three kaons event mixing ~(127-287) events
		long maxevents = 200000000000;
		ekpkpreqkmavgvz_data_v1 ana = new ekpkpreqkmavgvz_data_v1();
    ...
    //    */
       ...
       /*
        ...
        //threeKaon_eventMixing(lv_my_ftels, lv_my_fast_fkps, lv_my_slow_fkps, lv_my_fkms);
