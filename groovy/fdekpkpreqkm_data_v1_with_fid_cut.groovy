...
//import extended_clas12_kinematic_fitter.*;
...
public class fdekpkpreqkm_data_v1_with_fid_cut {
	//public int NFTElec;
	...
	public float efkpfkpfkm_MM_efkpfkpfkm,  efkpckpfkm_MM_efkpckpfkm,  eckpckpfkm_MM_eckpckpfkm;  
	...
	public H1F hi_efkpfkpfkm_MM_efkpfkpfkm, hi_efkpckpfkm_MM_efkpckpfkm, hi_eckpckpfkm_MM_eckpckpfkm;
	...
	public DataGroup dg_kinematics, dg_rec_electron, dg_rec_kp1, dg_rec_kp2, dg_rec_kps, dg_rec_km, dg_rec_xi, dg_rec_p, dg_rec_pim, dg_vtime, dg_fdtof, dg_cdtof, dg_vz, dg_tof_t, dg_tof_path, dg_counter;
	...
	public fdekpkpreqkm_data_v1_with_fid_cut() {
	...
	//counter for no of rec particles 
		...
		hi_efkpfkpfkm_MM_efkpfkpfkm = new H1F("hi-efkpfkpfkm-MM-efkpfkpfkm", "hi-efkpfkpfkm-MM-efkpfkpfkm", 25, 0.0, 2.4);//25, 0.0, 2.4 //25, 0.9, 1.4
		...
		hi_efkpckpfkm_MM_efkpckpfkm = new H1F("hi-efkpckpfkm-MM-efkpckpfkm", "hi-efkpckpfkm-MM-efkpckpfkm", 25, 0.9, 1.4);//25, 0.0, 2.4 //25, 0.9, 1.4
		hi_efkpckpfkm_MM_efkpckpfkm.setFillColor(LIGHTGREEN);
	...
	// ekpkpkm MM reconstruction
		...
		hi_kp_vz = new H1F("H-kp-vz", "H-kp-vz", 100, -20, 20);
		...
		hi_CD_pos_beta_mom = new H2F("H-CD-pos-beta-mom", "H-CD-pos-beta-mom", 50, 0.1, 3.0, 50, 0.1, 1.1);
		hi_CD_pos_beta_mom.setTitle("POS  #beta vs mom");
		hi_CD_pos_beta_mom.setTitleX("p (GeV)");
		hi_CD_pos_beta_mom.setTitleY("FTB #beta");
		hi_CD_neg_beta_mom = new H2F("H-CD-neg-beta-mom", "H-CD-neg-beta-mom", 50, 0.1, 3.0, 50, 0.1, 1.1);
		hi_CD_neg_beta_mom.setTitle("NEG  #beta vs mom");
		hi_CD_neg_beta_mom.setTitleX("p (GeV)");
		hi_CD_neg_beta_mom.setTitleY("FTB #beta");
		hi_CD_neutral_beta_mom = new H2F("H-CD-neutral-beta-mom", "H-CD-neutral-beta-mom", 50, 0, 3.0, 50, 0.1, 1.1);
		...
		hi_CD_pos_mass_mom = new H2F("H-CD-pos-mass-mom", "H-CD-pos-mass-mom", 100, 0, 4, 150, -0.5, 4.5);
		...
		hi_CD_neg_mass_mom = new H2F("H-CD-neg-mass-mom", "H-CD-neg-mass-mom", 100, 0, 4, 150, -0.5, 2.0);
		hi_CD_neg_mass_mom.setTitle("NEG Mass^2 vs mom");
		hi_CD_neg_mass_mom.setTitleX("p (GeV)");
		hi_CD_neg_mass_mom.setTitleY("M^2 (GeV^2)");
		hi_CD_neutral_mass_mom = new H2F("H-CD-neutral-mass-mom", "H-CD-neutral-mass-mom", 100, 0, 4, 150, -0.5, 2.0);
		...
		hi_CD_pos_mass_the = new H2F("H-CD-pos-mass-the", "H-CD-pos-mass-the", 150, 30, 180, 100, -0.5, 4.5);
		...
		hi_CD_neg_mass_the = new H2F("H-CD-neg-mass-the", "H-CD-neg-mass-the", 150, 30, 180, 100, -0.5, 2);
		...
		hi_CD_neutral_mass_the = new H2F("H-CD-neutral-mass-the", "H-CD-neutral-mass-the", 150, 30, 180, 100, -0.5, 2);
	...
	*/
	...
	public int makeFDElectron(DataBank bank, HipoDataBank cal_Bank, HipoDataBank track_Bank, HipoDataBank traj_Bank, HipoDataBank run_Bank, HipoDataBank cc_Bank) {
		...
		for(int k = 0; k < bank.rows(); k++){
            ...
            /*
            if (NFDElec == 1 
            	&& research_fitter.particle_test(k, bank) //// all particles required to be detected in FD for first publications
            	&& research_fitter.forward_detector_cut(k, bank)
            	&& research_fitter.calorimeter_energy_cut(k, cal_Bank)
            	&& research_fitter.calorimeter_diagonal_cut(k, p, cal_Bank)
            	&& research_fitter.electron_z_vertex_cut(vz)
            	&& research_fitter.dc_fiducial_cut(k, bank, track_Bank, traj_Bank, run_Bank)) { */
			 /*
			if (NFDElec == 1 
				&& research_fitter.particle_test(k, bank) 
				&& research_fitter.electron_test(k, p, vz, bank, cal_Bank, track_Bank, traj_Bank, run_Bank, cc_Bank)) { //*/
	...
	public void makeOthers(DataBank recbank, DataBank recSCBank, HipoDataBank cal_Bank, HipoDataBank track_Bank, HipoDataBank traj_Bank, HipoDataBank run_Bank, HipoDataBank cc_Bank) {
		...
		for (int k = 0; k < recbank.rows(); k++) {
			...
			if (pid == 321 ) {
				...
				for (int r = 0; r < recSCBank.rows(); r++) {
					if (recSCBank.getByte("detector", r) == 12 && inDC && fdChi2pidCut && research_fitter.dc_fiducial_cut(k, recbank, track_Bank, traj_Bank, run_Bank)) {	//&& fdkpMass2Cut
			...
			//((pid == 45 && vz > -9 && vz < 6) || (pid == 2212 && vz > -9 && vz < 6) || (pid == 321 && vz > -9 && vz < 6) || (pid == 211 && vz > -9 && vz < 6) || (pid == -11 && vz > -9 && vz < 6))
			if ( q > 0 && ((pid == 2212 && vz > -9 && vz < 6) || (pid == 321 && vz > -9 && vz < 6) || (pid == 211 && vz > -9 && vz < 6) ) ) {
				if (inDC && fdChi2pidCut && mom > 0.6 && research_fitter.dc_fiducial_cut(k, recbank, track_Bank, traj_Bank, run_Bank)) {
			...
			//((pid == -2212 && vz > -9 && vz < 6) || (pid == -321 && vz > -9 && vz < 6) || (pid == -211 && vz > -9 && vz < 6))
			if ( q < 0 && ((pid == -321 && vz > -9 && vz < 6) || (pid == -211 && vz > -9 && vz < 6))) {//|| || (pid == -11 && vz > -9 && vz < 6)
				if (inDC && fdChi2pidCut && mom > 0.6 && research_fitter.dc_fiducial_cut(k, recbank, track_Bank, traj_Bank, run_Bank)) {
...
//			H_FT_e_p_the.fill(e_the, e_mom);
	...
	public void plot(){
		...
		myCanvas = new EmbeddedCanvasTabbed("Kinematics","REC-electron","REC-kp1(fast)","REC-kp2(slow)","REC-kps","REC-km","MM-ekpkpkm","MM-ekpkpkm1","MM-kpAspip","MM-scatter","MM-lam-evnt","IM-kmlambda","MM-spectra","FD-TOF", "CD-TOF", "VTime", "particle-Vz","TOF-t","TOF-path", "Counter");
		...
		myCanvas.getCanvas("CD-TOF").getPad(0).getAxisX().setRange(0.2, 3.0);
		...
		myCanvas.getCanvas("CD-TOF").getPad(5).getAxisX().setRange(0.2, 3.0);
	...
	public static void main(String[] args) {
		...
		fdekpkpreqkm_data_v1_with_fid_cut ana = new fdekpkpreqkm_data_v1_with_fid_cut();
    ...
    //    */
       ...
       // */
        ...
        for (String runstrg : toProcessFileNames) if(count < maxevents){
            ...
            //research_fitter = new MyFitter(Eb); //My extended kinematic fitter
