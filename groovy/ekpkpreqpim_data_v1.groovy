...
//		}
	...
	public boolean select_efkpfkp(){
		boolean res = false;
		if(found_eFT && fkps.size() == 2){
			double avg_vz = (double)(fkps.get(0).vz() + fkps.get(1).vz() )/2;
			...
			//LorentzVector Vmissekpkp = new LorentzVector(0, 0, 0, 0);
			//Vmissekpkp.add(VT);Vmissekpkp.add(VB);Vmissekpkp.sub(Vftel_corrected.vector());Vmissekpkp.sub(fkps.get(0).vector());
