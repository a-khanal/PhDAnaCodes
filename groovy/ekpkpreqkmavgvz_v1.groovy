...
public class ekpkpreqkmavgvz_v1 {
	//public int NFTElec;
	...
	//ekpkpreqkm	reconstructed
		...
		hi_ekpkpkm_MM_ekpkp = new H1F("hi_ekpkpkm_MM_ekpkp", "hi_ekpkpkm_MM_ekpkp", 50, 1.5, 4.0); //50, 1.6, 2.1
    	...
    	hi_ekpkpkm_MM_ekpkp_nocorr = new H1F("hi_ekpkpkm_MM_ekpkp_nocorr", "hi_ekpkpkm_MM_ekpkp_nocorr", 50, 1.5, 4.0);//50, 1.6, 2.1
    ...
    //ekpkpkm generated
    	...
    	hi_mc_ekpkpkm_mm_ekpkp = new H1F("hi_mc_ekpkpkm_mm_ekpkp", "hi_mc_ekpkpkm_mm_ekpkp", 50, 1.5, 4.0); //50, 1.6, 2.1
...
// ekpkpkm MM reconstruction
		...
		hi_ekpkpkm_MM_ekpkpkm_nocorr.setTitleY("Events/[8 MeV/c^2]");
		hi_ekpkpkm_MM_ekpkpkm_nocorr.setFillColor(LIGHTGREEN);

		hi_ekpkpkm_IM_kmlambda = new H1F("hi_ekpkpkm_IM_kmlambda", "hi_ekpkpkm_IM_kmlambda", 50, 1.5, 4.0); //50, 1.6, 2.1
		...
		hi_ekpkpkm_IM_kmsigma = new H1F("hi_ekpkpkm_IM_kmsigma", "hi_ekpkpkm_IM_kmsigma", 50, 1.5, 4.0); //50, 1.6, 2.1
		...
		H_efkpckp_MM_req_km = new H1F("H_efkpckp_MM_req_km", "H_efkpckp_MM_req_km", 50, 1.5, 4.0);// 50, 1.6, 2.1
		...
		H_efkpckp_MM_req_ckm = new H1F("H_efkpckp_MM_req_ckm", "H_efkpckp_MM_req_ckm", 50, 1.5, 4.0);//50, 1.6, 2.1
		...
		H_ekpkp_MM_req_pim = new H1F("H_ekpkp_MM_req_pim", "H_ekpkp_MM_req_pim", 50, 1.5, 4.0);//50, 1.6, 2.1
		...
		H_efkpckp_MM_req_pim = new H1F("H_efkpckp_MM_req_pim", "H_efkpckp_MM_req_pim", 50, 1.5, 4.0); //50, 1.6, 2.1
		...
		H_efkpckp_MM_req_cpim = new H1F("H_efkpckp_MM_req_cpim", "H_efkpckp_MM_req_cpim", 50, 1.5, 4.0); //50, 1.6, 2.1
		...
		for (int k = 0; k < recbank.rows(); k++) {
			...
			if (ftbpid == 211 ) {
				...
				for (int r = 0; r < recSCBank.rows(); r++) {
					if (recSCBank.getByte("detector", r) == 12 && inDC && fdChi2pidCut) {	//&& fdChi2pidCut
			...
			if (ftbpid == 321 ) {
				...
				for (int r = 0; r < recSCBank.rows(); r++) {
					...
					if (recSCBank.getByte("detector", r) == 4 && recSCBank.getByte("detector", r) == 12) {
						nfdandcdpip++;
						hi_pip_counter.fill(nfdandcdpip);
					}
...
//		}
	...
	public boolean select_ekpkp_req_km(){
		boolean res = false;
		//ftels = new ArrayList<Particle>();
		if( found_eFT && fkps.size() >= 2 && fkms.size() >= 1 && mcels.size() ==1 && mckps.size() == 2 && mckms.size() == 1) {
			...
			// used generated phi for the detected electron to see if can achieve improvements with phi correction
			...
			Vmissekpkpkm_noCorr.add(VT);Vmissekpkpkm_noCorr.add(VB);Vmissekpkpkm_noCorr.sub(Ve);Vmissekpkpkm_noCorr.sub(reckp1.vector());Vmissekpkpkm_noCorr.sub(reckp2.vector());Vmissekpkpkm_noCorr.sub(fkms.get(0).vector());
			...
			Vmissekpkpkm.add(VT);Vmissekpkpkm.add(VB);Vmissekpkpkm.sub(Vftel_corrected.vector());Vmissekpkpkm.sub(reckp1.vector());Vmissekpkpkm.sub(reckp2.vector());Vmissekpkpkm.sub(fkms.get(0).vector());
			...
			if (Math.abs(ekpkpkm_MM_ekpkpkm - 1.192642) <= 0.052){
				...
				VxifromSigma.add(Vconstrainedsigma);VxifromSigma.add(fkms.get(0).vector());
...
//			H_FT_e_p_the.fill(e_the, e_mom);
		...
		if(select_ekpkp_req_km()) {
			...
			H_FT_e_t_f.fill(Math.toDegrees(Vftel_corrected.phi()), Math.toDegrees(Vftel_corrected.theta()));
			H_FT_e_p_f.fill(Math.toDegrees(Vftel_corrected.phi()), Vftel_corrected.p());
			H_FT_e_p_the.fill(Math.toDegrees(Vftel_corrected.theta()), Vftel_corrected.p());
			...
			hi_rec_km_dvx.fill((float)fkms.get(0).vx() - (float)mckms.get(0).vx());
			hi_rec_km_dvy.fill((float)fkms.get(0).vy() - (float)mckms.get(0).vy());
	...
	public static void main(String[] args) {
		...
		ekpkpreqkmavgvz_v1 ana = new ekpkpreqkmavgvz_v1();
